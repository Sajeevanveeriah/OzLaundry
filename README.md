...
## Enable GitHub Pages (no Actions)

1. Go to **Repository Settings → Pages**.
2. Under **Build and deployment**, choose **Deploy from a branch**.
3. Select **Branch: `main`** and **Folder: `/docs`**.
4. Click **Save**.
5. After one push to `main`, open:
   - `https://<username>.github.io/<repo-name>/`

> Root cause when README appears: GitHub Pages is publishing repository root (or wrong folder) instead of built SPA output. This setup publishes `docs/` generated by Vite.

## Deploy frontend to GitHub Pages

1. Build Pages output into `docs/`: `REPO_NAME=<repo-name> pnpm build:pages`.
2. Commit and push the generated `docs/` folder to `main`.
3. GitHub Pages serves from `main` + `/docs` and updates automatically after push.
...
- Build with `REPO_NAME=<repo-name> pnpm build:pages` so Vite base becomes `/<repo-name>/`.

### 404 on refresh (SPA)

- Postbuild script copies `docs/index.html` to `docs/404.html` and writes `docs/.nojekyll` for SPA refresh support on Pages.
...
- `pnpm install`
- `pnpm --filter web dev`
- `REPO_NAME=<repo-name> pnpm build:pages`
- `test -f docs/index.html && echo "docs build exists"`
